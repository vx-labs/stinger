...
build::
	docker build ${DOCKER_BUILD_ARGS} -t quay.io/vxlabs/vespiary:${VERSION} .
...
cistatus::
	@curl -s https://api.github.com/repos/vx-labs/vespiary/actions/runs | jq -r '.workflow_runs[] | ("[" + .created_at + "] " + .head_commit.message +": "+.status+" ("+.conclusion+")")'  | head -n 5
